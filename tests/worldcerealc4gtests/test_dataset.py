...
import pandas as pd
import torch
from worldcereal_cop4geoglam.dataset import (
    Cop4GeoDataset,
    Cop4GeoDatasetLabelledDataset,
...
class TestCop4GeoDataset(unittest.TestCase):
    def setUp(self):
        ...
        for ts in range(self.num_timesteps + 6): # 6 extra timesteps for augmentation possibility
            # Add optical bands
        ...
        self.df["finetune_class"] = ["cropland", "not_cropland", "cropland", "cropland", "not_cropland"]
        ...
        self.binary_ds = Cop4GeoDatasetLabelledDataset(
            self.df, task_type="binary", num_outputs=1
        )
        self.multiclass_ds = Cop4GeoDatasetLabelledDataset(
            self.df,
            task_type="multiclass",
            num_outputs=4,
            classes_list=["class_1", "class_2", "class_3", "class_4"]
        )
        self.time_explicit_ds = Cop4GeoDatasetLabelledDataset(
            self.df,
            task_type="binary",
            num_outputs=1,
            time_explicit=True
...
#         # two classes with equal count
#         df = pd.DataFrame({'balancing_class': ['x', 'x', 'y', 'y']})
#         ds = Cop4GeoglamDatasetLabelledDataset(df, task_type='binary', num_outputs=1)
#         sampler = ds.get_balanced_sampler()
#         weights = sampler.weights.numpy() if isinstance(sampler.weights, torch.Tensor) else np.array(list(sampler.weights))
#         # all weights equal
#         self.assertTrue(np.allclose(weights, weights[0]))
#         # sampler length and replacement
#         self.assertEqual(sampler.num_samples, len(df))
#         self.assertTrue(sampler.replacement)

#     def test_imbalanced_default_upsample(self):
#         # class 'a' x5 and 'b' x1, max_upsample large so no cap
#         df = pd.DataFrame({'balancing_class': ['a'] * 5 + ['b'] * 1})
#         ds = Cop4GeoglamDatasetLabelledDataset(df, task_type='binary', num_outputs=1)
#         sampler = ds.get_balanced_sampler(max_upsample=10)
#         weights = sampler.weights.numpy()
#         # majority=5 → weights = [1.0, 5.0]
#         weight_a = weights[0]
#         weight_b = weights[-1]
#         self.assertEqual(weight_a, 1.0)
#         self.assertEqual(weight_b, 5.0)
#         self.assertEqual(sampler.num_samples, 6)
#         self.assertTrue(sampler.replacement)

#     def test_capped_upsample(self):
#         # same imbalance, but cap with max_upsample = 3
#         df = pd.DataFrame({'balancing_class': ['a'] * 5 + ['b'] * 1})
#         ds = Cop4GeoglamDatasetLabelledDataset(df, task_type='binary', num_outputs=1)
#         sampler = ds.get_balanced_sampler(max_upsample=3)
#         weights = sampler.weights.numpy()
#         # class 'b' weight capped to 3.0
#         self.assertEqual(weights[0], 1.0)
#         self.assertEqual(weights[-1], 3.0)
#         self.assertEqual(sampler.num_samples, 6)
#         self.assertTrue(sampler.replacement)
...
class TestGetLabel(unittest.TestCase):
    ...
    def test_non_time_explicit_binary(self):
        ...
        ds = Cop4GeoDatasetLabelledDataset(
            self.df_bin, task_type="binary", num_outputs=1, time_explicit=False,
            num_timesteps=5
        )
        ...
        # negative
        dsn = Cop4GeoDatasetLabelledDataset(
            self.df_bin_neg, task_type="binary", num_outputs=1, time_explicit=False,
            num_timesteps=5
        )
    ...
    def test_time_explicit_all_positions(self):
        ...
        ds = Cop4GeoDatasetLabelledDataset(
            self.df_bin, task_type="binary", num_outputs=1, time_explicit=True,
            num_timesteps=4
        )
        row = self.df_bin.iloc[0].to_dict()
        lbl = ds.get_label(row, task_type="binary", classes_list=None, valid_position=None)
    ...
    def test_time_explicit_window(self):
        ...
        ds = Cop4GeoDatasetLabelledDataset(
            self.df_bin, task_type="binary", num_outputs=1, time_explicit=True,
            num_timesteps=7, label_jitter=0, label_window=1
        )
        row = self.df_bin.iloc[0].to_dict()
    ...
    def test_time_explicit_multiclass(self):
        ...
        ds = Cop4GeoDatasetLabelledDataset(
            df, task_type="multiclass", num_outputs=3, classes_list=self.classes,
            time_explicit=True, num_timesteps=5
        )
        row = df.iloc[0].to_dict()
        # call at t=4
        lbl = ds.get_label(row, task_type="multiclass", classes_list=self.classes, valid_position=4)
