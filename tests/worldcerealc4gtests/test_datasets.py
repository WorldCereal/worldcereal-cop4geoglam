...
class TestCop4GeoDataset(unittest.TestCase):
    def setUp(self):
        ...
        self.multiclass_ds = Cop4GeoLabelledDataset(
            self.df,
            task_type="multiclass",
            num_outputs=4,
            classes_list=["cropland", "not_cropland", "other1", "other2"],
        )
        self.time_explicit_ds = Cop4GeoLabelledDataset(
...
class TestBalancedSampler(unittest.TestCase):
    def test_equal_frequencies(self):
        ...
        ds = Cop4GeoLabelledDataset(df, task_type="binary", num_outputs=1)
        sampler = ds.get_balanced_sampler(sampling_class="balancing_class")
        weights = (
            sampler.weights.numpy()
            if isinstance(sampler.weights, torch.Tensor)
            else np.array(list(sampler.weights))
        )
        # all weights equal
    ...
    def test_imbalanced(self):
        # class 'a' x5 and 'b' x1, default min/max weights, "balanced" method
        ...
        weight_a = weights[0]
        weight_b = weights[-1]
        self.assertAlmostEqual(weight_b / weight_a, 5.0, delta=0.01)
        self.assertEqual(sampler.num_samples, 6)
        self.assertTrue(sampler.replacement)

        # same imbalance, but use log balancing, custom cap and switch off normalization
        df = pd.DataFrame({"balancing_class": ["a"] * 99 + ["b"] * 1})
        ds = Cop4GeoLabelledDataset(df, task_type="binary", num_outputs=1)
        sampler = ds.get_balanced_sampler(
            sampling_class="balancing_class",
            method="log",
            clip_range=(0.2, 1),
            normalize=False,
        )
        weights = sampler.weights.numpy()
        # class 'b' weight capped to 1
        self.assertAlmostEqual(weights[-1], 1, delta=0.001)
        # class 'a' weight capped to 0.2
        self.assertAlmostEqual(weights[0], 0.2, delta=0.001)
