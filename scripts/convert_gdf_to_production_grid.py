...
def parse_args(argv: Iterable[str]) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "Add per-feature UTM geometry (S2-based) and attach an external identifier as tile_name"
        )
    )
    p.add_argument("--input", required=True, help="Input GeoParquet path")
    p.add_argument(
        "--id-source",
        required=True,
        help="Name of column providing unique identifier to copy into tile_name",
    )
    p.add_argument("--output", help="Output GeoParquet (default: <input>_utm.parquet)")

    return p.parse_args(argv)  # type: ignore
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    out_path = (
        Path(ns.output)
        if ns.output
        else in_path.with_name(in_path.stem + "_utm.parquet")
    )

    print(f"Loading GeoDataFrame: {in_path}")
    ...
    enriched = enrich_with_utm(
        gdf,
        web_mercator_grid=ns.web_mercator_grid,
    )
