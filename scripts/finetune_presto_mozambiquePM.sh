...
FINETUNE_SCRIPT="scripts/training/finetuning/finetune_presto.py"
...
ARGS=(
    ...
    --croptype_classes_key "CROPTYPE_Mozambique"
    ...
    # Pretrained encoder checkpoint path or URL. Must be a compatible Presto model
    --pretrained_model_path "/vitodata/worldcereal/data/COP4GEOGLAM/mozambique_pm/models/WorldCerealPresto-dualtask-SeasonalMultiTaskLoss-LC10-CT24-Global-TwoSeasons-LINEAR-month-augment=True-balance=True-timeexplicit=True-masking=enabled-ema0.2-clamp=0.05-5.0-run=202604212323_encoder.pt"
    ...
    # After each training epoch a shadow EMA model is updated:
    #   ema = (1 - alpha) * ema + alpha * current_weights
    # Validation and checkpointing always use the EMA model, which is
    # inherently smooth (it averages recent weight states), so no separate
    # loss-level smoothing is needed.
