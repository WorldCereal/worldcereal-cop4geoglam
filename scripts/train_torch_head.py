...
class LinearHead(nn.Module):
    def __init__(self, in_dim: int, num_classes: int):
        ...
        logger.info(f"LinearHead: in_dim={in_dim}, num_classes={num_classes}")
...
class TorchHeadTrainer:
    ...
    def _plot_confusion_matrices(
        ...
        label_ids = np.arange(len(self.classes_list), dtype=np.int64)
    ...
    def evaluate(self, model: nn.Module, test_loader: DataLoader) -> Dict[str, float]:
        ...
        metrics["F1"] = round(f1_score(labels_all, preds_all, average="macro"), 3)
...
def main() -> None:
    ...
    if USE_MANUAL_CONFIG:
        ...
        class ManualArgs:
            def __init__(self):
                ...
                self.presto_model_path = "/projects/worldcereal/COP4GEOGLAM/moldova/models/presto-prometheo-cop4geoglam-new-splits-month-CROPTYPE_Moldova-augment=False-balance=True-timeexplicit=False-run=202601131552/presto-prometheo-cop4geoglam-new-splits-month-CROPTYPE_Moldova-augment=False-balance=True-timeexplicit=False-run=202601131552_encoder.pt"
                self.data_dir = "/projects/worldcereal/COP4GEOGLAM/moldova/models/presto-prometheo-cop4geoglam-new-splits-month-CROPTYPE_Moldova-augment=False-balance=True-timeexplicit=False-run=202601131552/"
                self.output_dir = (
                    "/projects/worldcereal/COP4GEOGLAM/moldova/models/croptype/"
                )
                self.modelversion = f"151-MDA_{self.head_type}_prestorun=202601131552"
                ...
                self.sampling_class = "finetune_class"
    ...
    trainer = TorchHeadTrainer(
        ...
        output_dir=Path(args.output_dir) / args.modelversion,
