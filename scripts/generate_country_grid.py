...
class _BoundingBox(NamedTuple):
    ...
    def from_polygon(cls, polygon: Union[MultiPolygon, Polygon], crs: Optional[int] = None) -> "_BoundingBox":
        ...
        crs_val = 4326 if crs is None else crs
        west, south, east, north = polygon.bounds
        return cls(west, south, east, north, crs=crs_val)
...
blocks_moldova = split_area(
    aoi=moldova_borders.geometry.values[0],
    projection=f'EPSG:{epsg_dict[country]}',
    tile_size=area_extension
)

# Intersect area with country borders to filter blocks fully outside the country
blocks_moldova_gdf = gpd.GeoDataFrame(geometry=blocks_moldova, crs=f'EPSG:{epsg_dict[country]}')
filtered_blocks = blocks_moldova_gdf.sjoin(moldova_borders, how='inner', predicate='intersects')
