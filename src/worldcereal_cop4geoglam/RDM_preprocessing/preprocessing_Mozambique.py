...
'''
Script used to preprocess the data for Mozambique before uploading to the RDM.
@Copernicus4GEOGLAM
'''
...
keep_columns = ["_id","id_psu","box_id","id_ssu","landuse","croptype","cropping_pattern","date","irrigation","geometry"]
...
#join datasets
dataset = croptype_points.merge(lu_points, on = ["box_id","id_ssu"], how='right')
...
#Save croptype_est for mapping later on
croptype_est["ewoc_croptype"]= None
croptype_est["landuse_mapping"] = None
croptype_est["croptype_mapping"] = None

croptype_est.to_csv(os.path.join(path, f"{activation}_croptype_est.csv"), index=False)
