...
class Cop4GeoDatasetLabelledDataset(Cop4GeoDataset):
    ...
    def __getitem__(self, idx):
        ...
        label = self.get_label(
            row, task_type=self.task_type, classes_list=self.classes_list, valid_position=valid_position - timestep_positions[0]
        )
    ...
    def get_label(
        ...
        if task_type == "binary":
            label[0, 0, valid_idx, 0] = int(not row_d["finetune_class"].startswith("not_"))
        elif task_type == "multiclass":
            if not classes_list:
                raise ValueError(
                    "classes_list should be provided for multiclass task"
                )
            label[0, 0, valid_idx, :] = np.array([int(row_d[col]) for col in classes_list])

        return label

def get_balanced_sampler(self, max_upsample: int = 10):
    ...
