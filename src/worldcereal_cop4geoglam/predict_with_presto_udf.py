...
def predict_with_presto(
    ...
    try:
        ...
        # Check if we have the expected 12 timesteps - but now with graceful handling
        if len(inarr.t) != 12:
            ...
            return create_nan_output_array(inarr, parameters['num_outputs'], error_msg)
        ...
        # Catch any exception and return NaN array with error info
        error_msg = f"Error in predict_with_presto: {str(e)}. " \
                   f"Input array shape: {inarr.shape if 'inarr' in locals() else 'N/A'}, " \
                   f"Timesteps: {len(inarr.t) if 'inarr' in locals() else 'N/A'}, " \
                   f"EPSG: {epsg}"
        logger.error(error_msg, exc_info=True)
        
        # Return NaN array if we have the input array, otherwise re-raise
        if 'inarr' in locals() and 'parameters' in locals():
            return create_nan_output_array(inarr, parameters.get('num_outputs', 1), error_msg)
        else:
            # If we can't create a proper NaN array, re-raise the exception
            raise
...
def apply_udf_data(udf_data: UdfData) -> UdfData:
    ...
    logger.info(f"Processing patch - Shape: {arr.shape}, "
                f"Timesteps: {len(arr.t)}, "
                f"Bands: {arr.bands.values.tolist()}, "
                f"Spatial dims: x={len(arr.x)}, y={len(arr.y)}")
